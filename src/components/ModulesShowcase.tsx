...
const ModulesShowcase = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsOpened(sectionProgress > 0.3);
    };
  ...
  return (
    <section ref={sectionRef} id="modules" className="py-32 bg-gradient-to-br from-slate-50 via-white to-blue-50 relative overflow-hidden">
      {/* Laptop closed view overlay */}
      <div 
        className={`absolute inset-0 z-50 transition-all duration-1000 ${
            ...
            >
              <div className="w-full h-full bg-black rounded-t-lg m-1 relative overflow-hidden">
                ...
                <img 
                  ...
                  className="w-full h-full object-cover rounded-t-lg opacity-70"
                />
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-center">
                  <div className="text-sm opacity-70 bg-black/50 px-3 py-1 rounded">Scroll to explore modules</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Main content with laptop frame when opened */}
      <div 
                  ...
                  {category.modules.map((module, moduleIndex) => (
                    <Dialog key={moduleIndex} open={openDialog === `${categoryIndex}-${moduleIndex}`} onOpenChange={(open) => setOpenDialog(open ? `${categoryIndex}-${moduleIndex}` : null)}>
                      <DialogTrigger asChild>
                        <div className="group bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-105 border border-gray-100/50 overflow-hidden cursor-pointer relative">
